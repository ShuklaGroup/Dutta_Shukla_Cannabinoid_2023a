...
if __name__=='__main__':
    ...
    axs.set_yscale('log')
    count = 1
    box = plt.boxplot([clas_diss[0,:],clas_diss[1,:]],positions=[count,count+1],widths = 0.6)
    ...
    count += 3
    print(nps_diss[0,:],nps_diss[1,:])
    box = plt.boxplot([nps_diss[0,:],nps_diss[1,:]],positions=[count,count+1],widths = 0.6)
    for item in ['whiskers', 'caps']:
        #print(item,box[item])
        plt.setp(box[item][0], color="Blue")
        plt.setp(box[item][1], color="Blue")
    ...
    axs.set_yticks([1e5,1e3,1e1], ['10$^5$','10$^3$','10$^0$'],**hfont, fontsize=24)

    axs.legend([box["boxes"][0], box["boxes"][1]], ['MSM', 'TRAM'],loc='lower right',fontsize=24)
    plt.ylabel('Dissociation time ($\mu$s)',**hfont, fontsize=30)
    
    plt.tight_layout()
    plt.savefig('Main_Figure_4C.png',dpi=300)
    plt.close()
