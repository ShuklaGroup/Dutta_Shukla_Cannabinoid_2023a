...
def density_plot(prob_dens):
    ...
    plt.tight_layout()
    plt.savefig('./Main_Figure_8A.png',transparent=False,dpi =300)
...
if __name__=='__main__': 
    feature_name = 'TM36'
    axis_label = {'TM36':'TM3-TM6 distance'}
    axis_lim = {'TM36':[8,20]}

    feature_list = []
    weights_list = []
    prob_dens = np.empty([2,3,50])
    for i,ligand in enumerate(['nps','classical']):
        for j,bt in enumerate(['0','1','2']):
            feature = pickle.load(open('./feature_files/CB1_' + ligand + '_TM36_95_bt_' + bt + '.pkl','rb'))
