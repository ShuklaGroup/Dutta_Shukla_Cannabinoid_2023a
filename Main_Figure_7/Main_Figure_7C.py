...
def kl_calculation(r,q):
    totdist = []
    ref_filename = './kl_div/CB1_classical_' + r + '_0_chv.npy'
    txx_ref = 1/np.load(ref_filename)

    query_filename = './kl_div/CB1_classical_' + q + '_0_chv.npy'
    ...
    np.save('./kl_div/CB1_classical_'+ r + '_' + q + '_kl_avg_score.npy',kl_avg)

    return kl_avg

def pdb_b_factor(r,q,j):
    f = open('./kl_div/CB1_classical_ref_' + q +'_0.pdb','r')
    fh = open('./kl_div/CB1_classical_ref_'+ r + '_' + q +'_b.pdb','w')
...
if __name__=='__main__':
    ref_state = 'I1'
    top = './kl_div/CB1_classical_strip.prmtop'
    a = np.load('./kl_div/CB1_classical_I1_0_chv_ind.npy')
    indexs = np.unique(a)
    weights_per_residue = np.empty([len(indexs),5])

    for i,state in enumerate(['I1','Bound','I2','I3']):
        filename = './kl_div/CB1_classical_' + state + '_0_chv.npy'
    
    for j,state2 in enumerate(['I1','Bound','I2','I3']):
        kl_file = './kl_div/CB1_classical_' + ref_state +  '_' + state2 + '_kl_avg_score.npy'
